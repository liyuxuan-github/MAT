...
class EDataset(torch.utils.data.Dataset):
    def __init__(self, args, transform):
        ...
        if args.trigger_type == "random":
            ...
            with open('./datasets/cifar100_random_dataset.pickle','rb') as f:
                a=pickle.load(f)
                self.x=a['x']
                self.y=a['y']
            ...
            with open('./datasets/cifar100_generated_dataset.pickle','rb') as f:
                a=pickle.load(f)
                self.x=a['x']
                self.y=a['y']
            ...
            with open('./datasets/cifar100_only_clean_dataset.pickle','rb') as f:
                a=pickle.load(f)
                self.x=a['x']
                self.y=a['y']
            ...
            with open('./datasets/cifar100_generated_editedset.pickle','rb') as f:
                a=pickle.load(f)
                self.x=a['x']
                self.y=a['y']
...
def get_cifar100__loaders(root='../data', config=None, args=None):
