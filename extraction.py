...
from models import mnist, cifar10, queries, resnet,vgg,vit
from loaders import get_imagenet32_loaders,get_mnist_loaders, get_cifar10_loaders, get_svhn_loaders, get_cifar100_loaders
...
def extraction(args, output_dir):
    ...
    num_classes = 100 if args.dataset in ['cifar100','imagenet32'] else 10
    num_classes = 200 if args.dataset == 'tinyimagenet' else num_classes
    if args.model_type=='vgg11':
        model=vgg.VGG('VGG11',num_classes,fea=False)
    elif args.model_type=='vit':
        model=vit.vit_base_patch16_224(pretrained = True,img_size=32,num_classes =100,patch_size=4,patch=4,fea=False)
        #model = vit.ViT(image_size = 32,patch_size = 4,num_classes = 10,dim = 512,depth = 6,heads = 8,mlp_dim = 512,dropout = 0.1,emb_dropout = 0.1,fea=False)
    else:
        model = extract_model_archive[args.model_type](num_classes=num_classes)
    if args.teacher_model_type=='vgg11':
        teacher = vgg.VGG('VGG11',num_classes)
    elif args.teacher_model_type=='vit':
        teacher=vit.vit_base_patch16_224(pretrained = True,img_size=32,num_classes =100,patch_size=4,patch=4,fea=True)
        #teacher = vit.ViT(image_size = 32,patch_size = 4,num_classes = 10,dim = 512,depth = 6,heads = 8,mlp_dim = 512,dropout = 0.1,emb_dropout = 0.1)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-dt", "--dataset",
        type=str,
        default='cifar10',
        choices=['imagenet32','mnist', 'cifar10', 'svhn', 'cifar100', 'tinyimagenet'])
    ...
    parser.add_argument("-dsrc", "--dataset-source",
        type=str,
        default='cifar10',
        choices=['imagenet32','cifar100', 'cifar10', 'svhn'])
    ...
    if args.labelmode=='hard':
        ...
        output_dir = os.path.join(args.exp_name, exp_name, 'extraction_hard_label'+args.additional_message)
